...
func NewRocksDB(path string, cacheSize, maxOpenFiles int, parser bchain.BlockChainParser, metrics *common.Metrics, spendingIndex bool) (d *RocksDB, err error) {
	...
	r := &RocksDB{path, db, wo, ro, cfh, parser, nil, metrics, c, maxOpenFiles, connectBlockStats{}, spendingIndex, sync.Mutex{}, sync.Mutex{}, make(map[string]*unpackedAddrContracts)}
...
func (d *RocksDB) LoadInternalState(config *common.Config) (*common.InternalState, error) {
	...
	if len(data) == 0 {
		is = &common.InternalState{Coin:  config.CoinName, UtxoChecked: true,
			SortedAddressContracts:  true,
			SpendingIndex:           d.spendingIndex,
